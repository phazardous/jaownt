								   ...
								   const int flags, qhandle_t *modelList, const int modelIndex, const int blendTime, const int currentTime)
{
	...
	if (!fileName[0])
	{
		mod_m = TRM_GetModelByHandle(modelList[modelIndex]);
	}
	else
	{
		mod_m = TRM_GetModelByHandle(TRM_RegisterModel(fileName));
	}
...
#define RAG_WAS_EVETRM_RENDERED		(0x02000)		// not particularily reliable, more of a hint
...
//#define		GHOUL2_RESERVED_FOTRM_RAGDOLL 0x0ff0  // these are not defined here for dependecies sake
...
int G2_Find_Bone_Rag(CGhoul2Info *ghlInfo, boneInfo_v &blist, const char *boneName)
{
	...
	currentModel = TRM_GetModelByHandle(TRM_RegisterModel(ghlInfo->mFileName));
	assert(currentModel);
	animModel =  TRM_GetModelByHandle(currentModel->mdxm->animIndex);
	assert(animModel);
			...
			if (index>=0&&index<(int)blist.size())
			{
				...
				if (bone.boneNumber>=0)
				{
					if (bone.flags & BONE_ANGLES_RAGDOLL)
					{
						...
						parms->me->s.pos.trType = TTRM_GRAVITY;
	...
	for(size_t i=0; i<blist.size(); i++)
	{
		...
		if (bone.boneNumber>=0)
		{
			...
			if ((bone.flags & BONE_ANGLES_RAGDOLL) || (bone.flags & BONE_ANGLES_IK))
			{
				...
				{
					bone.RagFlags&=~RAG_WAS_NOT_RENDERED;
					bone.RagFlags|=RAG_WAS_EVETRM_RENDERED;
					numRendered++;
				}
				if	(bone.RagFlags&RAG_PCJ_PELVIS)
				{
...
{
  	model_t		*mod_m = TRM_GetModelByHandle(TRM_RegisterModel(ghoul2->mFileName));
	model_t		*mod_a = TRM_GetModelByHandle(mod_m->mdxm->animIndex);
