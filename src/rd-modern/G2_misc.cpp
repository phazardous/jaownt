...
#define GOTRM_TAG_UPPER (256)
#define GOTRM_TAG_MASK (~255)

static int CurrentTag=GOTRM_TAG_UPPER+1;
static int CurrentTagUpper=GOTRM_TAG_UPPER;
...
#define MAX_GOTRM_RECORDS (500)

int AllocGoreRecord()
{
	while (GoreRecords.size()>MAX_GOTRM_RECORDS)
	{
		int tagHigh=(*GoreRecords.begin()).first&GOTRM_TAG_MASK;
		...
		{
			if (((*GoreRecords.begin()).first&GOTRM_TAG_MASK)!=tagHigh)
			{
				break;
			}
			it = GoreRecords.begin();
...
void ResetGoreTag()
{
	...
	CurrentTagUpper+=GOTRM_TAG_UPPER;
}
...
{
	int			i, x;
  	model_t		*mod_m = TRM_GetModelByHandle(TRM_RegisterModel(fileName));
...
void G2_List_Model_Bones(const char *fileName, int frame)
{
	...
	mdxaSkelOffsets_t	*offsets;
  	model_t			*mod_m = TRM_GetModelByHandle(TRM_RegisterModel(fileName));
	model_t			*mod_a = TRM_GetModelByHandle(mod_m->mdxm->animIndex);
...
qboolean G2_GetAnimFileName(const char *fileName, char **filename)
{
	...
	model_t				*mod = TRM_GetModelByHandle(TRM_RegisterModel(fileName));
...
void TRM_TransformEachSurface( const mdxmSurface_t *surface, vec3_t scale, IHeapAllocator *G2VertSpace, size_t *TransformedVertsArray,CBoneCache *boneCache)
{
	int				 j, k;
	mdxmVertex_t 	*v;
					...
					CBoneCache *boneCache, const model_t *currentModel, int lod, vec3_t scale, IHeapAllocator *G2VertSpace, size_t *TransformedVertArray, bool secondTimeAround)
{
	...
	if (!offFlags)
	{
		...
		TRM_TransformEachSurface(surface, scale, G2VertSpace, TransformedVertArray, boneCache);
	}
...
#define MAX_GOTRM_VERTS (3000)
static SVertexTemp GoreVerts[MAX_GOTRM_VERTS];
static int GoreIndexCopy[MAX_GOTRM_VERTS];
static int GoreTouch=1;

#define MAX_GOTRM_INDECIES (6000)
static int GoreIndecies[MAX_GOTRM_INDECIES];

#define GOTRM_MARGIN (0.0f)
...
void G2_GorePolys( const mdxmSurface_t *surface, CTraceSurface &TS, const mdxmSurfHierarchy_t *surfInfo)
{
	...
	int flags=15;
	assert(numVerts<MAX_GOTRM_VERTS);
	for ( j = 0; j < numVerts; j++ )
	{
		int pos=j*5;
		vec3_t delta;
	...
	for ( j = 0; j < numTris; j++ )
	{
		...
		assert(newNumTris*3+3<MAX_GOTRM_INDECIES);
		for (k=0;k<3;k++)
		{
