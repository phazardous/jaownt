...
{
#ifndef _G2_LISTEN_SERVETRM_OPT
	return qfalse;
#else
...
void G2API_CleanGhoul2Models(CGhoul2Info_v **ghoul2Ptr)
{
	if (*ghoul2Ptr)
	{
		...
		extern int TRM_GetRNumEntities(void);
		extern void TRM_SetRNumEntities(int num);
		//check if this instance is actually on a refentity
		int i = 0;
		int r = TRM_GetRNumEntities();
		...
		while (i < r)
		{
			if ((CGhoul2Info_v *)backEndData->entities[i].e.ghoul2 == *ghoul2Ptr)
			{
				...
				TRM_SetRNumEntities(0); //avoid recursive error
...
qhandle_t G2API_PrecacheGhoul2Model( const char *fileName )
{
	if ( G2_ShouldRegisterServer() )
		return TRM_RegisterServerModel( fileName );
	else
		return TRM_RegisterModel( fileName );
}
...
{
	model_t		*mod_m = TRM_GetModelByHandle(modelIndex);
...
char *G2API_GetGLAName(CGhoul2Info_v &ghoul2, int modelIndex)
{
	if (G2_SetupModelPointers(ghoul2))
	{
		if ((ghoul2.size() > modelIndex))
		{
			//model_t	*mod = TRM_GetModelByHandle(TRM_RegisterModel(ghoul2[modelIndex].mFileName));
...
qboolean G2_TestModelPointers(CGhoul2Info *ghlInfo) // returns true if the model is properly set up
{
	...
	if (ghlInfo->mModelindex != -1)
	{
		...
		{
			ghlInfo->mModel = TRM_RegisterServerModel(ghlInfo->mFileName);
		}
		else
		{
			ghlInfo->mModel = TRM_RegisterModel(ghlInfo->mFileName);
		}
		ghlInfo->currentModel = TRM_GetModelByHandle(ghlInfo->mModel);
		if (ghlInfo->currentModel)
		{
			if (ghlInfo->currentModel->mdxm)
			{
				...
				ghlInfo->animModel = TRM_GetModelByHandle(ghlInfo->currentModel->mdxm->animIndex);
