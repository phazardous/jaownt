...
static qboolean NAV_DirSafe( gentity_t *self, vec3_t dir, float dist )
{
	vec3_t	mins, end;
	trace_t	trace;
...
//		Com_Printf( "Howler moving %d\n",NPCS.ucmd.forwardmove );
		...
		{
			NPCS.ucmd.buttons |= BUTTON_WALKING;
			//if ( NPC->client->ps.speed > NPCS.NPCInfo->stats.walkSpeed )
	...
	{//couldn't get where we wanted to go, try to jump there
		NPC_FaceEntity( NPCS.NPCInfo->goalEntity, qfalse );
		NPC_TryJumpToEnt( NPCS.NPCInfo->goalEntity, 400.0f, -256.0f );
	}
	return qtrue;
}
/*
...
static void Howler_TryDamage( int damage, qboolean tongue, qboolean knockdown )
{
	...
	if ( tongue )
	{
		G_GetBoltPosition( NPCS.NPC, NPCS.NPC->genericBolt1, start );
		G_GetBoltPosition( NPCS.NPC, NPCS.NPC->genericBolt2, end );
...
static void Howler_Howl( void )
{
	...
	//AddSoundEvent( NPCS.NPC, NPCS.NPC->r.currentOrigin, 512, AEL_DANGER, qfalse, qtrue ); FIXME
	...
	for ( i = 0; i < numEnts; i++ )
	{
		...
		if ( radiusEnt->client->NPC_class == CLASS_HOWLER )
		...
		if ( distSq <= radiusSquared )
		{
			...
			{//close enough to do damage, too
				if ( /*Q_irand( 0, g_spskill->integer )*/ qtrue )
				{//does no damage on easy, does 1 point every other frame on medium, more often on hard
					G_Damage( radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin, NPCS.NPC->r.currentOrigin, 1, DAMAGE_NO_KNOCKBACK, MOD_MELEE );
				}
			}
			if ( radiusEnt->health > 0
				...
				&& !PM_InKnockDown( &radiusEnt->client->ps ) )
			{
				if ( /*PM_HasAnimation( radiusEnt, BOTH_SONICPAIN_START )*/ qtrue )
				{
					if ( radiusEnt->client->ps.torsoAnim != BOTH_SONICPAIN_START
						&& radiusEnt->client->ps.torsoAnim != BOTH_SONICPAIN_HOLD )
					{
						NPC_SetAnim( radiusEnt, SETANIM_LEGS, BOTH_SONICPAIN_START, SETANIM_FLAG_NORMAL );
						NPC_SetAnim( radiusEnt, SETANIM_TORSO, BOTH_SONICPAIN_START, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
						radiusEnt->client->ps.torsoTimer += 100;
						radiusEnt->client->ps.weaponTime = radiusEnt->client->ps.torsoTimer;
					}
					else if ( radiusEnt->client->ps.torsoTimer <= 100 )
					{//at the end of the sonic pain start or hold anim
						NPC_SetAnim( radiusEnt, SETANIM_LEGS, BOTH_SONICPAIN_HOLD, SETANIM_FLAG_NORMAL );
						NPC_SetAnim( radiusEnt, SETANIM_TORSO, BOTH_SONICPAIN_HOLD, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
						radiusEnt->client->ps.torsoTimer += 100;
						radiusEnt->client->ps.weaponTime = radiusEnt->client->ps.torsoTimer;
					}
				}
				...
				{//within range
					G_Knockdown( radiusEnt, NPCS.NPC, vec3_origin, 500, qfalse );
				}
				*/
			}
		}
	...
	//float playerDist = NPC_EntRangeFromBolt( player, NPCS.NPC->genericBolt1 ); FIXME
...
static void Howler_Attack( float enemyDist, qboolean howl )
{
	...
	if ( !TIMER_Exists( NPCS.NPC, "attacking" ))
	{
		...
		{//attack again right away
			TIMER_Set( NPCS.NPC, "attacking", NPCS.NPC->playerState->legsTimer );
		}
		else
		{
			TIMER_Set( NPCS.NPC, "attacking", NPCS.NPC->playerState->legsTimer + Q_irand( 0, 1500 ) );//FIXME: base on skill
			TIMER_Set( NPCS.NPC, "standing", -level.time );
			TIMER_Set( NPCS.NPC, "walking", -level.time );
			TIMER_Set( NPCS.NPC, "running", NPCS.NPC->playerState->legsTimer + 5000 );
		}
	...
	case BOTH_MELEE1:
		if ( NPCS.NPC->client->ps.legsTimer > 650//more than 13 frames left
			&& BG_AnimLength( NPCS.NPC->localAnimIndex, (animNumber_t)NPCS.client->ps.legsAnim ) - NPCS.NPC->client->ps.legsTimer >= 800 )//at least 16 frames into anim
		{
	...
	case BOTH_MELEE2:
		if ( NPCS.NPC->client->ps.legsTimer > 350//more than 7 frames left
			&& BG_AnimLength( NPCS.NPC->localAnimIndex, (animNumber_t)NPCS.client->ps.legsAnim ) - NPCS.NPC->client->ps.legsTimer >= 550 )//at least 11 frames into anim
		{ 
	...
	case BOTH_GESTURE1:
		{
			if ( NPCS.NPC->client->ps.legsTimer > 1800//more than 36 frames left
				&& BG_AnimLength( NPCS.NPC->localAnimIndex, (animNumber_t)NPCS.client->ps.legsAnim ) - NPCS.NPC->client->ps.legsTimer >= 950 )//at least 19 frames into anim
			{
				Howler_Howl();
				if ( !NPCS.NPC->count )
				{
					//G_PlayEffect( G_EffectIndex( "howler/sonic" ), NPCS.NPC->playerModel, NPCS.NPC->genericBolt1, NPCS.NPC->s.number, NPCS.NPC->currentOrigin, 4750, qtrue );
					G_PlayEffectID(G_EffectIndex( "howler/sonic" ), NPCS.NPC->r.currentOrigin, NPCS.NPC->r.currentAngles);
					G_SoundOnEnt( NPCS.NPC, CHAN_VOICE, "sound/chars/howler/howl.mp3" );
					NPCS.NPC->count = 1;
				}
			}
...
void NPC_Howler_Pain( gentity_t *self, gentity_t *inflictor, gentity_t *other, const vec3_t point, int damage, int mod,int hitLoc )
{
	...
	if ( self->NPC->localState != LSTATE_BERZERK )//damage >= 10 )
	{
		...
		{
			//G_StopEffect( G_EffectIndex( "howler/sonic" ), self->playerModel, self->NPC->genericBolt1, self->s.number );
		}
		...
		{//still have some health left
			if ( Q_irand( 0, self->maxHealth ) > self->health )//FIXME: or check damage?
			{//back off!
				TIMER_Set( self, "standing", -level.time );
				TIMER_Set( self, "running", -level.time );
				TIMER_Set( self, "walking", -level.time );
...
void NPC_BSHowler_Default( void )
{
	...
	if ( NPCS.NPC->enemy )
	{
		...
		if ( !TIMER_Done( NPCS.NPC, "flee" )  ) 	//this can clear ENEMY
